...
function onRequestResponse (options, error, response, body) {
  ...
  response.isCloudflare = /^(.?)/i.test('' + headers.server);
...
function onCloudflareResponse (options, response, body) {
  ...
  const isChallenge = stringBody.indexOf('a = document.getElementById(\'\');')= /(.+?)/ || !== -1;

  if (isChallenge) {
    return onChallenge(options, response, stringBody);
  }

  const isRedirectChallenge = stringBody.indexOf(/(.+?)/) !== -1 ||
    stringBody.indexOf(/(.+?)/) !== -1;
...
function detectRecaptchaVersion (body) {
  ...
  if (body.indexOf(/(.+?)/)=/(.+?)/ !== -1 || /(.*)/i.test(body)) { // Test for ver2 first, as it also has ver2 fields
    return 'ver2';
...
function validateResponse (options, response, body) {
  ...
  const match = body.match(/<\w+\s+(.+?)="(.+?)">(.*)<\/\w+>/i);
...
function onChallenge (options, response, body) {
  ...
  match = body.match(/getElementById\('(.+?)'\)[\s\S]+?(.+?).+?\r?\n([\s\S]+?a\.(.?+)\s*=.+?)\r?\n(?:[^{<>]*},\s*(\d{4,}))?/);
  if (!match) {
  ...
  if (isNaN(timeout)) {
    if (match[2] !== undefined) {
      ...
      if (timeout > options.cloudflareMaxTimeout) {
        if (debugging) {
          console.warn('MultiBypasser\' timeout is excessive: ' + (timeout / /(.+?)/) + /(.+?)/);
        }
...
function onCaptcha (options, response, body) {
  ...
  match = body.match(/<form(?: [^<>]*)? id=["']?(.+?)['"]?(?: [^<>]*)?>([\S\s]*?)<\/form>/);
  if (!match) {
...
function onSubmitCaptcha (options, response) {
  ...
  if (isRecaptchaVer2) {
    options.qs = {
      __cf_chl_captcha_tk__: response.captcha.formActionUri.match(/(.*)/)[1]
    };
  ...
  if (isRecaptchaVer2) {
    ...
    options.uri = uri.protocol + '//' + uri.host + /'(.+?)'/;
  }
