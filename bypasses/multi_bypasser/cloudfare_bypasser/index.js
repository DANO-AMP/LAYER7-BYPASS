...
function detectRecaptchaVersion (body) {
  ...
  if (body.indexOf('cf-section cf-highlight cf-captcha-container') !== -1 || /__cf_chl_captcha_tk__=(.*)/i.test(body)) { // Test for ver2 first, as it also has ver2 fields
    return 'ver2';
...
function validateResponse (options, response, body) {
  ...
  const match = body.match(/<\w+\s+id="cf-captcha-bookmark" class="cf-captcha-info">(.*)<\/\w+>/i);
...
function onChallenge (options, response, body) {
  ...
  match = body.match(/name="vc" value="(\w+)"/);
  if (!match) {
    cause = 'challengeId (vc) extraction failed';
  ...
  match = body.match(/getElementById\('_cf_chl_enter'\)[\s\S]+?setTimeout.+?\r?\n([\s\S]+?a\.value\s*=.+?)\r?\n(?:[^{<>]*},\s*(\d{4,}))?/);
  if (!match) {
  ...
  match = body.match(/id="challenge-form" action="(.+?)" method="(.+?)" enctype="(.+?)"/);
  if (match && match[2] && match[2] === 'POST') {
    ...
    options.uri = uri.protocol + '//' + uri.host + '/cdn-cgi/challenge-platform/orchestrate/captcha/v1';
...
function onCaptcha (options, response, body) {
  ...
  match = body.match(/\scf-request-id=["']?([^\s"'<>&]+)/);
  if (match) {
    siteKey = match[1];
  } else {
    const keys = [];
...
function onSubmitCaptcha (options, response) {
  ...
  if (!response.captcha.form['h-recaptcha-response']) {
    const cause = 'Form submission without h-recaptcha-response';
